...
| <a name="input_sku"></a> [sku](#input\_sku) | FortiGate SKU to use directly. If provided, this will be used instead of `image_sku` and validation against the fortinet_sku_to_versions_map.json will be automatically skipped. Azure still requires `image_version` separately. Use this to deploy image versions not yet in the supported versions map. | `string` | `null` | no |
